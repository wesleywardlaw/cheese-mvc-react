...
class MenuAddCheeseForm extends Component {
  ...
  render = () => {
    ...
    const availableCheeses = allCheeses.filter(cheese => uniqueCheese(cheese, currentCheeses));
    ...
    return (
      <Form>
        <Form.Row>
          <Form.Group as={Col}>
            <Form.Control as='select' name='cheeseID' value={cheeseID}
...
MenuAddCheeseForm.propTypes = {
  ...
  currentCheeses: PropTypes.arrayOf(cheesePropType).isRequired,
};
